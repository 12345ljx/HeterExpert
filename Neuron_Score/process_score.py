...
def plot_neuron_score(domains_data):
    ...
    for ax, layer_idx in zip(axs.flat, range(num_layers)):
        ...
        ax.set_xticklabels([f'D{i}' for i in range(num_domains)], fontsize=20)
        yticks = np.linspace(0, np.max(domains_data), 3)
...
def plot_neuron_score_sub(domains_data):
    num_layers = domains_data.shape[0]
    num_neurons = domains_data.shape[1]
    num_domains = domains_data.shape[2]
    ...
    for ax, layer_idx in zip(axs.flat, [7, 11]):
        for i, neuron_idx in enumerate(neurons_idx):
            values = get_neuron_scores(layer_idx, neuron_idx)
            ax.fill(angles, values, color=colors[i], alpha=0.25)
            ax.plot(angles, values, color=colors[i], linewidth=2)
        ...
        ax.set_xticks(angles[:-1])
        ax.set_xticklabels([f'D{i}' for i in range(num_domains)], fontsize=23)
        yticks = np.linspace(0, 0.005, 5)
        ytick_labels = [f'{t:.3f}' for t in yticks] 
        ax.set_yticks(yticks)
        ax.set_yticklabels(ytick_labels, color="grey", size=20)
        
    axs.flat[0].set_ylim(0, 0.005)
    axs.flat[1].set_ylim(0, 0.0035)
