...
def load_lora(model, args):
    from architectures.nlp import GemmaWrapper, LLaMAWrapper
    
    if args.lora_args is not None:
        lora_config = LoraConfig(
            task_type=TaskType.CAUSAL_LM,
            inference_mode=False,
            **args.lora_args.peft_kwargs,
        )
        if isinstance(model, GemmaWrapper):
            model.gemma = get_peft_model(model.gemma, lora_config)
        # elif isinstance(model, LLaMAWrapper):
...
def merge_lora(model):
    ...
    if isinstance(model, GemmaWrapper):
        if isinstance(model.gemma, PeftModel):
            model.gemma = model.gemma.merge_and_unload(progressbar=True, safe_merge=True, adapter_names=['default'])
    if isinstance(model, LLaMAWrapper):
        if isinstance(model.llama, PeftModel):
            model.llama = model.llama.merge_and_unload(progressbar=True, safe_merge=True, adapter_names=['default'])
    else:
...
def load_model(args, exp_name: str, exp_id: str, device: Union[torch.device, str] = 'cpu'):
    ...
    for arg, model_arg in zip(reversed(run_args), reversed(run_model_args)):
        if arg.base_on is None:
            ...
            # TODO refactor this so that no duplication here is needed
            model = load_lora(model, args)
            if 'relu' in arg.dsti_enforce_mode:
                from architectures.moe.dsti import replace_with_relu
                from architectures.moe.dsti import find_gelu_activations
                from architectures.moe.dsti import find_non_relu_activations
                activations_to_sparsify = find_non_relu_activations(model, **model_arg)
                model = replace_with_relu(model, activations_to_sparsify)
            model = model.to(device)
