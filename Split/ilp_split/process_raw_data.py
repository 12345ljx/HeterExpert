...
function_name = 'domains(task_type)'

output_dir = f'/usr/workdir/HeterExpert/Split/model_split/ilp/{model_name}/{function_name}/{NUM_EXPERT}'
os.makedirs(output_dir, exist_ok=True)

data = np.load(f'/usr/workdir/HeterExpert/Neuron_Importance/score/task_type/importance_score_reduced_{NUM_MODULES}.npz')
module2neurons = data['module2neurons'].astype(np.int32)  # [num_layers, num_modules, num_neurons]

for layer_idx in range(NUM_HIDDEN_LAYERS):
    placement = np.load(f'/usr/workdir/HeterExpert/Split/ilp_split/raw_data/{model_name}/{function_name}/k{NUM_EXPERT_ACT}n{NUM_EXPERT}m{NUM_MODULES}/neuron_grouping.layer{layer_idx}.npz')['placement']  # [num_neurons,]
