...
def main():
    ...
    DOMAIN_NUM = 8
    NUM_EXPERT = 8
    NUM_EXPERT_SHARING = 1
    dff_hidden_size = 8192
    num_hidden_layers = 16
    size_expert = dff_hidden_size // NUM_EXPERT
    num_sharing = size_expert * NUM_EXPERT_SHARING
    ...
    for layer_idx in range(num_hidden_layers):
        ...
        for i in range(NUM_EXPERT_SHARING):
            experts[i] = neurons_sharing[i * size_expert: (i + 1) * size_expert]
            
        for i in range(NUM_EXPERT_SHARING, NUM_EXPERT):
            remain = neurons_remaining[(i - NUM_EXPERT_SHARING)::(NUM_EXPERT - NUM_EXPERT_SHARING)][:]
        ...
        output_path = f'/usr/workdir/HeterExpert/Split/model_split/residual/{model_name}/{function_name}/{NUM_EXPERT}'
