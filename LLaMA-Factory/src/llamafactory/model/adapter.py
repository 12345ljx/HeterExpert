...
def _setup_freeze_tuning(
    ...
    for idx in trainable_layer_ids:
        if moe_args.training_gate_layers and idx not in moe_args.training_gate_layers:
            continue
        trainable_layers.append(".{:d}.mlp.gate.z_logits".format(idx))
        trainable_layers.append(".{:d}.mlp.gate.w_logits".format(idx))
        trainable_layers.append(".{:d}.mlp.gate.weight_noise".format(idx))
        trainable_layers.append(".{:d}.mlp.gate.scaling".format(idx))
        trainable_layers.append(".{:d}.mix_gate.z_logits".format(idx))
        trainable_layers.append(".{:d}.mix_gate.w_logits".format(idx))
...
def init_adapter(
    ...
    if finetuning_args.finetuning_type == "full":
        ...
        _setup_freeze_tuning(model, moe_args, finetuning_args, is_trainable, cast_trainable_params_to_fp32)
