...
if TYPE_CHECKING:
    ...
    from ...hparams import DataArguments, FinetuningArguments, GeneratingArguments, ModelArguments, MoEArguments
...
def run_sft(
    ...
    model = load_model(tokenizer, model_args, moe_args, finetuning_args, training_args.do_train)
    ...
    elif finetuning_args.training_parts == "mix_gate":
        checkpoints_path = f"{base_path}/{str(moe_args)}/{model_name}:{task_name}(seed={training_args.seed},lr={training_args.learning_rate})"
    else:
        raise NotImplementedError
    
    if moe_args.moeficate:
        checkpoints_path = os.path.join(checkpoints_path, str(moe_args))
    
    checkpoints_path = os.path.join(checkpoints_path, str(int(time.time())))
    training_args.output_dir = checkpoints_path 
