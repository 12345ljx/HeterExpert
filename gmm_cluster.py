...
def analyse_cluster(estimator, domains_num: int, sample_count: int, random_seed: int, base_path: str):
    embeddings, labels = get_tasks_embeddings(sample_count, random_seed, write_file=False)
    clusters = estimator.predict(normalize(embeddings))
    
    unique_labels = np.unique(labels)
    domains_compose = np.zeros((domains_num, len(unique_labels)), dtype=np.int32)
    ...
    np.save(os.path.join(base_path, 'domains_compose.npy'), domains_compose)
...
def main():
    ...
    domains_num = 8
    cluster_sample_count = 1000
    matryoshka_dim = 128
    embeddings, labels = get_tasks_embeddings(cluster_sample_count, random_seed, matryoshka_dim, write_file=True)
    embeddings = normalize(embeddings)
    
    # kmeans = KMeans(n_clusters=domains_num, random_state=33)
    # kmeans.fit_predict(normalize(embeddings))
    
    base_path = '/usr/workdir/HeterExpert/domains/gmm_cluster(dim=128)'
    os.makedirs(base_path, exist_ok=True)
    estimator = GaussianMixture(n_components=domains_num, covariance_type='full', max_iter=150, init_params='kmeans')
    estimator.fit(embeddings)
        
    
    analyse_cluster(estimator, domains_num, cluster_sample_count, random_seed, base_path)
    ...
    for task_path in tqdm(d for d in directory.iterdir() if d.is_dir()):
    ...
    embeddings_2d, centers_2d = dim_reduction(embeddings_all, centers, 'tsne', random_seed)
    plot(embeddings_2d, tasks_labels_all, clusters_labels_all, centers_2d, os.path.join(base_path, 'tasks_distribution.pdf'))
    ...
    for idx, domain in enumerate(domains_data_merge):
        domain.save_to_disk(os.path.join(base_path, f'domain{idx}'))
